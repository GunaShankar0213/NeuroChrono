...
class TransMorph(nn.Module):
    def __init__(self, config):
        ...
        # We change the output of up3 to 16 (reg_head_chan) instead of 64 (embed_dim//2)
        self.up3 = DecoderBlock(embed_dim, config.reg_head_chan, skip_channels=embed_dim//2 if if_convskip else 0, use_batchnorm=False)
        self.up4 = DecoderBlock(config.reg_head_chan, config.reg_head_chan, skip_channels=config.reg_head_chan if if_convskip else 0, use_batchnorm=False)
