...
class HDBETPipeline:
    ...
    def __init__(
        ...
        # FIX: Robust stem extraction for .nii.gz or .nii
        ...
        # Intermediate files
        self.bet_output = self.output_dir / f"{self.stem}_bet.nii.gz"
        self.bet_mask_output = self.output_dir / f"{self.stem}_bet_mask.nii.gz"

        # Final deliverables
        self.final_output = self.output_dir / f"{self.stem}_bet_cropped.nii.gz"
        self.qc_report = self.output_dir / f"{self.stem}_qc_report.png"
    ...
    def _save_qc_report(
        ...
        # Rotate 90 degrees for better viewing orientation
    ...
    def run(self) -> None:
        ...
        # FIX: Check for already processed files
        if "_bet" in self.input_file.name:
            error_msg = (
                f"Dangerous Input Detected: {self.input_file.name}\n"
                "It appears you are feeding an already skull-stripped file ('_bet') "
                "back into the pipeline. This will cause over-stripping and data loss.\n"
                "Please use the raw T1/T0 input file."
            )
        ...
        # FIX: Success flag for safe cleanup
        success = False
        ...
        try:
            ...
            time.sleep(3) # Short buffer for filesystem sync
            ...
            # FIX: Mark as successful only if we reach here
            success = True
            ...
            # FIX: Only delete intermediate files if the process was SUCCESSFUL
            if success:
                ...
                self.logger.warning("Pipeline failed or interrupted!")
                self.logger.warning(f"Preserving intermediate file for debugging: {self.bet_output}")
...
python Modules/Module1/preprocess/hd_bet_pipeline.py --input Data/raw/OASIS_Fast/T0.nii.gz --output-dir processed --keep-intermediate --tta
